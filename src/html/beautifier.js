...
Printer.prototype.print_space_or_wrap = function(text) {
  if (this.wrap_line_length) {
    if (
      this._output.current_line.get_character_count() + text.length + 1 >=
...
Beautifier.prototype.beautify = function() {
  ...
  /////////////////这里是为了计算多少属性
  var n_tokens = new Tokenizer(source_text, this._options).tokenize();
  var n_raw_token = n_tokens.next();
  var atrr_num = 0;
  var __arrrr = [];
  while (n_raw_token.type !== TOKEN.EOF) {
    if (
  ...
  while (raw_token.type !== TOKEN.EOF) {
    if (raw_token.type === TOKEN.TAG_OPEN || raw_token.type === TOKEN.COMMENT) {
      ...
      parser_token.attr_all_count = __arrrr.shift();
...
Beautifier.prototype._handle_tag_close = function(
  ...
  // console.log();
  // if(raw_token.previous.type == 'TK_VALUE'){
...
Beautifier.prototype._handle_inside_tag = function(
  ...
  if (last_tag_token.is_unformatted) {
    ...
    if (
      printer._output.space_before_token &&
      ...
      if (raw_token.type === TOKEN.ATTRIBUTE) {
        ...
        if (this._is_wrap_attributes_force) {
          ...
          if (
            this._is_wrap_attributes_force_expand_multiline &&
            last_tag_token.attr_all_count ===
              this._options.wrap_attributes_count
          ) {
...
Beautifier.prototype._print_custom_beatifier_text = function(
  ...
  if (raw_token.text !== '') {
    ...
    if (last_tag_token.tag_name === 'script') {
      _beautifier =
        typeof this._js_beautify === 'function' && this._js_beautify;
    } else if (last_tag_token.tag_name === 'style') {
      _beautifier =
        typeof this._css_beautify === 'function' && this._css_beautify;
    }
    ...
    if (_beautifier) {
      ...
      var _level =
        white.match(/[^\n\r]*$/)[0].split(this._options.indent_string).length -
        1;
  ...
  if (!parser_token.is_empty_element) {
    if (parser_token.is_end_tag) {
      ...
      if (
        ...
        parser_token.custom_beautifier = uses_beautifier(
